...
if ( ! class_exists( 'WPPL_Meta' ) ) {
	...
	class WPPL_Meta implements WPPL_Object {
		 ...
		 * @return ?WPPL_Meta
		 */
		public static function factory( string $object_type, string $meta_key, string $object_subtype ): ?WPPL_Meta {
			global $wp_meta_keys;
			...
			if ( isset( $wp_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
				...
				if ( ! isset( static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
					static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] =
						new WPPL_Meta( $meta_key, $object_type, $args );
				}
		...
		public function unregister() {
			if ( $this->object_type && $this->object_subtype ) {
				unregister_meta_key( $this->object_type, $this->meta_key, $this->object_subtype );
			}
		}
